...
export class DependencyTreeSlicer {
    ...
    public static calcTimeSlices(parentEntity: IEntity, minDate: Date, maxDate: Date, minPflegeStatus: PflegeStatus): Timespan[] {
        return VersionMerger.getEntityTimeline(parentEntity, minPflegeStatus)
            .filter(parentSlice => parentSlice.getVon() <= maxDate && parentSlice.getBis() >= minDate) // filter by min/max date
            .reduce((totalSlices, parentSlice) => { // merge timeline slices into one list
                // limit slice min/max dates
                const sliceVon = parentSlice.getVon() > minDate ? parentSlice.getVon() : minDate;
                const sliceBis = parentSlice.getBis() < maxDate ? parentSlice.getBis() : maxDate;

                if (parentSlice.getVersion().getChildEntities().length === 0) {
                    // if no children: add parent slice
                    return DependencyTreeSlicer.intersectTimeSlices(totalSlices, [new Timespan(sliceVon, sliceBis)]);
                } else {
                    // else: add child slices recursively
                    return DependencyTreeSlicer.intersectTimeSlices(totalSlices, parentSlice.getVersion()
                        .getChildEntities()
                        .map(childEntity => DependencyTreeSlicer.calcTimeSlices(childEntity, sliceVon, sliceBis, minPflegeStatus))
                        .reduce((totalChildSlices, currentChildSlice) => {
                            return DependencyTreeSlicer.intersectTimeSlices(totalChildSlices, currentChildSlice);
                        }, [])
                    );
                }},
                []
    ...
    public static intersectTimeSlices(slices1: Timespan[], slices2: Timespan[]): Timespan[] {
        ...
        const vonList2 = vonList.concat(bisList
            .filter((bisTs, idx, list) => idx < list.length - 1)
